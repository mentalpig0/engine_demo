...
Shader::Shader(const char* vertexPath, const char* fragmentPath) : ID(0) {
    if (!load(vertexPath, fragmentPath)) {
        std::cerr << "ERROR::SHADER::Failed to create shader from files: "
...
bool Shader::load(const char* vertexPath, const char* fragmentPath) {
    ...
    try {
        ...
        std::cerr << "ERROR::SHADER::FILESYSTEM_ERROR: " << e.what() << std::endl;
        return false;
    }
    ...
    return loadFromSource(vShaderCode, fShaderCode);
}
